...
public class WrapperTest extends MiaoshaDemoV2ApplicationTests {
    ...
    public void test() {
        try {
            Wrapper<User> wrapper = new QueryWrapper<User>().lambda().eq(User::getName, 123)
                    .or(c -> c.eq(User::getSalt, 1).eq(User::getId, 2))
                    .eq(User::getId, 1);
    ...
    @Test
    public void test2() {
        LambdaQueryWrapper<User> ew = new LambdaQueryWrapper<User>()
                .eq(User::getSalt, 123)
                .and(i -> i.eq(User::getName, 65444)
                        .le(User::getName, 66666))
                .ne(User::getAccount, 222);
        log(ew.getSqlSelect());
        log(ew.getSqlSet());
        log(ew.getSqlSegment());
    }
